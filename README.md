...
> 当前版本：v0.7.3（2026-07-31）

面向高中生的 AI 数学学习系统。基于 DeepSeek V4 多老师路由与 Solver → Verifier → Formatter 三段流水线，同时提供多模态切题、手把手教学、个人题库、题单、错题与组卷系统。
...
| `step_difficulty` | 单步难度（level + score） | 难度徽标 |
...
另有一个「武亮难度系数」= 所有步骤难度分之和，随 `overall_difficulty` 一起返回。
...
| V0.8+ | 母题分析、AI 生题、OCR 批改、整卷批改、学情规划、教师端 | 📋 规划中 |
